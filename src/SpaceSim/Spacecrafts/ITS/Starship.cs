...
namespace SpaceSim.Spacecrafts.ITS
{
    class Starship : SpaceCraftBase
    {
        ...
        double _payloadMass = 0;
        double _dryMass = 120000;
        ...
        protected override void RenderShip(Graphics graphics, Camera camera, RectangleF screenBounds)
        {
            ...
            if (rollAngle <= 90)
                graphics.DrawImage(this.Texture, screenBounds.X - screenBounds.Width * 0.1f, screenBounds.Y, screenBounds.Width * 1.2f, screenBounds.Height);
            else
                graphics.DrawImage(this.Texture, screenBounds.X + screenBounds.Width * 0.1f, screenBounds.Y, -screenBounds.Width * 1.2f, screenBounds.Height);
            ...
            if (Settings.Default.WriteCsv && (DateTime.Now - timestamp > TimeSpan.FromSeconds(1.07)))
            {
                string filename = MissionName + ".csv";

                if (!File.Exists(filename))
                {
                    //File.AppendAllText(filename, "Velocity (m/s), Orbital (m/s), Acceleration (cm/s²), Orbital (cm/s²), Altitude (dm), Drag (kN), Lift (kN), Heating (dW/m²)\r\n");
                    File.AppendAllText(filename, "Velocity(m/s), Acceleration(cm/s²), Altitude(m), Throttle (%)\r\n");
                }

                timestamp = DateTime.Now;
                
                double velocity = GetRelativeVelocity().Length();
                double inertialVelocity = GetInertialVelocity().Length();
                double acceleration = GetRelativeAcceleration().Length();
                double inertialAcceleration = GetInertialAcceleration().Length();
                double altitude = GetRelativeAltitude();
                //double drag = AccelerationD.Length();
                //double lift = AccelerationL.Length();
                //double fwdFlap = GetDihedral(0);
                //double aftFlap = GetDihedral(1);
                ...
                //string contents = string.Format("{0}, {1}, {2}, {3}, {4}, {5}, {6}, {7}\r\n",
                //    velocity,
                //    inertialVelocity,
                //    acceleration * 100,
                //    inertialAcceleration * 100,
                //    altitude / 100,
                //    drag * 1000,
                //    lift * 1000,
                //    this.HeatingRate / 10);
