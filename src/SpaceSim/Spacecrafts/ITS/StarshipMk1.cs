...
{
    class StarshipMk1 : SpaceCraftBase
    {
        public override string CraftName { get { return "Starship Mk1"; } }
        public override string CommandFileName { get { return "Starship.xml"; } }

        public override double DryMass { get { return 105000; } }
        //public override double DryMass { get { return 200000; } }
        ...
        protected override void RenderShip(Graphics graphics, Camera camera, RectangleF screenBounds)
        {
            ...
            if (rollAngle <= 90)
                ...
                graphics.DrawImage(this.Texture, screenBounds.X + screenBounds.Width * 1.0f, screenBounds.Y, -screenBounds.Width * 1.0f, screenBounds.Height);
            ...
            if (Settings.Default.WriteCsv && (DateTime.Now - timestamp > TimeSpan.FromSeconds(1.03)))
            {
                ...
                if (!File.Exists(filename))
                {
                    ...
                    File.AppendAllText(filename, "Velocity (m/s), Acceleration (dm/s2), Altitude (dm), Mach (x100), Heating (dW/m2)\r\n");
                }

                timestamp = DateTime.Now;

                double velocity = this.GetRelativeVelocity().Length();
                double altitude = this.GetRelativeAltitude();
                double speedOfSound = this.GravitationalParent.GetSpeedOfSound(altitude);
