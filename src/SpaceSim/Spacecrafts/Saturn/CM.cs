...
{
    class CM : SpaceCraftBase
    {
        public override string CraftName { get { return "Command Module"; } }
        public override string CommandFileName { get { return "CM.xml"; } }

        public override double Width { get { return 3.9; } }
        public override double Height { get { return 3.5; } }

        public override double DryMass { get { return 5506; } } // Apollo 8
        // public override double DryMass { get { return 5560; } } // Apollo 11
        ...
        public CM(string craftDirectory, DVector2 position, DVector2 velocity, double payloadMass, double propellantMass = 120)
            : base(craftDirectory, position, velocity, payloadMass, propellantMass, "Saturn/CM.png")
        {
        ...
        protected override void RenderShip(Graphics graphics, Camera camera, RectangleF screenBounds)
        {
            ...
            if (Settings.Default.WriteCsv && (DateTime.Now - timestamp > TimeSpan.FromSeconds(seconds)))
            {
                string filename = MissionName + ".csv";

                if (!File.Exists(filename))
                {
                    File.AppendAllText(filename, "Velocity (m/s), Acceleration (cm/s²), Altitude (dam), Throttle (‰), Pressure, Heating\r\n");
                }
